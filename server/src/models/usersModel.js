...
const getUserChangePassword = async (data, option) => {
  try {
    const db = await GET_DB();
    return await db.collection('users').findOne({ [option]: data });
  } catch (error) {
    throw new Error(error);
  }
};
...
const changePassword = async (id, password) => {
  try {
    const db = await GET_DB();
    return await db.collection('users').updateOne({ _id: new ObjectId(id) }, { $set: { password } });
  } catch (error) {
    throw new Error(error);
  }
